...
| BGE-M3 | 79.6% | 91.2% | 94.6% | 0.863 | 零样本；CPU ~20× 快于 Qwen3 |

详见 `docs/实验记录.md` §6.5 / §6.6。

### BGE-M3 vs Qwen3-Embedding-0.6B

同协议零样本对比（难例 12 / 未见 500 / Benchmark 1000）：

```bash
python run_compare_bge_m3_qwen3.py
```

| 数据集 | BGE-M3 R@1 | Qwen3 R@1 | 延迟优势 |
|--------|------------|-----------|----------|
| 未见 500（主结论） | 79.6% | **88.6%** | BGE-M3 ~20× 更快（47 vs 938 ms/q） |
| Benchmark 1000 | 90.2% | **96.4%** | BGE-M3 ~21× 更快 |
| 难例 12 | 75.0% | **91.7%** | BGE-M3 ~20× 更快 |

完整报告：`results/comparison_bge_m3_vs_qwen3.json` / `.csv`
